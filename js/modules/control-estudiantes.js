(function () {
  ...
  function renderCards(estudiantes) {
    ...
    grid.innerHTML = estudiantes.map(function (estudiante, idx) {
      ...
      return (
        '<article class="control-estudiantes__card">' +
          ...
          '<div class="control-estudiantes__actions">' +
            '<button type="button" class="btn btn--primary control-estudiantes__approve" data-index="' + idx + '">' +
              'Aprobar' +
            '</button>' +
          '</div>' +
        '</article>'
  ...
  function initApproveAction() {
    ...
    grid.addEventListener('click', function (event) {
      ...
      var card = btn.closest('.control-estudiantes__card');
      var status = card ? card.querySelector('.control-estudiantes__status') : null;
      if (status) {
  ...
  async function loadEstudiantes() {
    try {
      var response = await fetch('/api/registro-estudiantes', { method: 'GET' });
      var data = await response.json();
      if (!response.ok) {
        throw new Error('No se pudo consultar la lista de estudiantes.');
      }

      renderCards(data.estudiantes || []);
    } catch (error) {
